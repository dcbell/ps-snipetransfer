### Connection info. Set URL and API personal access token.
$SnipeURL = Get-Content -Path './secure/url.txt' -Raw
$ApiKey = Get-Content -Path './secure/key.txt' -Raw
...
function New-SnipeCheckin {
	...
	switch ($CheckinType) {
		"asset" { 
			...
			$ApiUrl = "$SnipeURL/api/v1/hardware/$CheckinID/checkin" 
		...
		"accessory" { 
			$ApiUrl = "$SnipeURL/api/v1/accessories/$CheckinID/checkin"
...
function New-SnipeCheckout {
	...
	switch ($CheckoutType) {
		"asset" { 
			$ApiUrl = "$SnipeURL/api/v1/hardware/$CheckoutID/checkout" 
		...
		"accessory" { 
			$ApiUrl = "$SnipeURL/api/v1/accessories/$CheckoutID/checkout"
...
function New-SnipeTransferAll {
	...
	$AccessoryIDs | foreach-object {
		...
		$ApiUrl = "$SnipeURL/api/v1/accessories/$($_.id)/checkedout"
